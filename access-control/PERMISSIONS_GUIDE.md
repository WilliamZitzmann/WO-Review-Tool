...
**Lost password / locked out**: both "Forgot password?" on the login screen and admin-assisted "reset password" (Groups/Root Accounts tabs) require Resend to be configured (`RESEND_API_KEY`/`RESEND_FROM_EMAIL`) — there is no temp-password fallback for a reset, only for brand-new account creation. "Forgot password?" gives the same enumeration-resistant response either way, so it never reveals whether an email has an account. Admin-assisted reset requires authority over that account's bucket (or root) — and if the account belongs to more than one group (multi-group membership), authority over *all* of them, since one reset changes the single password shared across every group. Without Resend configured, "reset password" fails outright (400) instead of showing a temporary password — set up Resend first. A logged-in account can also change its own password any time via the header's "Change password" button, given the current one.
